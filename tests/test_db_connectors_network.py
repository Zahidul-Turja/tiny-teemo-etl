"""
Tests for MySQL and PostgreSQL connector behaviour — specifically around
connection failures, bad hostnames, and the exact error that was seen in
production:

    Can't connect to MySQL server on 'tinyteemo-mtsql-local'
    ([Errno -2] Name or service not known)

These tests use unittest.mock so they run without a live database.
They verify that:
  1. A DNS / hostname failure raises ConnectionError with a meaningful message.
  2. Wrong credentials raise ConnectionError (not an unhandled exception).
  3. test_connection() always returns a dict — never raises.
  4. The connector rejects obviously wrong hostnames early (e.g. container_name
     used instead of the Docker service name).

A separate section covers the most common misconfiguration pattern:
connecting from inside Docker using the container_name instead of the
service name (the bug that produced the reported error).
"""
...
class TestPostgreSQLConnectionFailures:
    def test_bad_hostname_raises_connection_error(self):
        ...
        with patch.object(connector, "connect", side_effect=ConnectionError("Failed to connect to PostgreSQL: Name or service not known")):
            with pytest.raises(ConnectionError) as exc_info:
                connector.connect()
...
class TestUploadRetryBehaviour:
    ...
    def test_all_retries_exhausted_raises(self):
        ...
        connector = MagicMock()
        connector.upload_dataframe.side_effect = ConnectionError("Name or service not known")
