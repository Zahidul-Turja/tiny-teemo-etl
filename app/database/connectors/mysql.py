"""
MySQL connector using PyMySQL — pure-Python, works on all platforms.

PyMySQL's API is nearly identical to mysql-connector-python:
  - DictCursor is passed at cursor creation time instead of cursor(dictionary=True)
  - Everything else (placeholders, commit/rollback, executemany) is the same.
"""
...
class MySQLConnector(BaseDatabaseConnector):
    ...
    def create_index(
        ...
        with self._conn.cursor() as cursor:
            cursor.execute(
                f"CREATE INDEX `{index_name}` ON `{table_name}` ({col_str})"
            )
        self._conn.commit()
