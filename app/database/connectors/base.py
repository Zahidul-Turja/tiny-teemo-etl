...
Provides:
  - upload_dataframe()  — the main entry point used by tasks.py
  - sanitize_df()       — converts pandas/numpy types to plain Python
                          so psycopg2/PyMySQL never choke on exotic types
...
class BaseDatabaseConnector(ABC):
    ...
    @abstractmethod
    def create_table(self, table_name: str, column_mappings: List[ColumnMapping]) -> None: ...

    @abstractmethod
    def drop_table(self, table_name: str) -> None: ...

    @abstractmethod
    def insert_data(self, table_name: str, df: pd.DataFrame, batch_size: int) -> Dict[str, Any]: ...
