"""
Celery task definitions for TinyTeemo ETL.

Features
--------
- Idempotent: re-submitting the same request reuses the existing job_id.
- Exponential-backoff retry (up to MAX_RETRIES).
- Dead-letter routing after exhausting retries.
- Per-stage progress published to Redis → WebSocket clients.
"""
...
def _fail_job(job_id: str, message: str) -> ETLJobResult:
    ...
    save_job(result)
    publish_progress(job_id, {"job_id": job_id, "stage": "failed", "progress": 100, "message": message})
    return result
    ...
    def on_failure(self, exc, task_id, args, kwargs, einfo):
        if isinstance(exc, MaxRetriesExceededError):
            ...
            etl_dead_letter.apply_async(
                kwargs={"job_id": job_id, "request_dict": request_dict, "reason": str(einfo)},
                queue="etl.dlq",
            )
...
def run_etl_task(
    ...
    from app.services.schema_mapper import Aggregator, DataValidator, RowFilter, SchemaMapper
    ...
    with ETLLogger(job_id=job_id) as etl_log:
        try:
            ...
            if request.api_source:
                ...
                etl_log.info(f"Extracting from DB: {src.connection.db_type.value} / {label}")
                df, auto_mappings = read_from_db(src)
                if not request.column_mappings:
                    request = request.model_copy(update={"column_mappings": auto_mappings})
                else:
            ...
            total_rows = len(df)
            _progress(job_id, "extract", 20, f"Extracted {total_rows} rows", total_rows=total_rows)

            # ── 2. FILTER ─────────────────────────────────────────────────────
            if request.filters:
                _progress(job_id, "filter", 30, f"Applying {len(request.filters)} filter rule(s)")
                df, filtered_out = RowFilter().apply(df, request.filters)
                _progress(job_id, "filter", 35, f"{len(df)} kept, {len(filtered_out)} discarded")
            ...
            if mapper.transformation_errors:
                for err in mapper.transformation_errors:
                    etl_log.warning(f"Transform warning on '{err['column']}': {err['error']}")
                _progress(job_id, "transform", 55,
            ...
            if request.validation_rules:
                _progress(job_id, "validate", 60, f"Running {len(request.validation_rules)} validation rule(s)")
                df, invalid_df, validation_errors = DataValidator().validate(df, request.validation_rules)
                failed_rows = len(invalid_df)
                if failed_rows > 0:
                    invalid_rows_file = etl_log.save_invalid_rows(invalid_df, validation_errors)
                _progress(job_id, "validate", 70, f"{len(df)} valid, {failed_rows} invalid", failed_rows=failed_rows)
            ...
            if request.db_destination:
                ...
                _progress(job_id, "load", 82, f"Writing to DB table '{dest.table_name}'")
            ...
            save_job(final)
            _progress(job_id, "done", 100, "ETL job completed successfully",
                      processed_rows=len(df), failed_rows=failed_rows)
            ...
            if attempt <= max_r and not isinstance(exc, (ValueError, FileNotFoundError, KeyError)):
                # Transient failure — will be retried; tell the UI
                _progress(job_id, "retrying", 0,
...
def _read_from_api_local(source, etl_log) -> "pd.DataFrame":
    ...
    with httpx.Client(timeout=30) as client:
        while next_url:
            ...
            etl_log.info(f"Fetched page {page}: +{len(records)} (total: {len(all_records)})")
...
def _db_upload_with_retry(connector, df, table_name, column_mappings,
                           if_exists, batch_size, max_retries, logger) -> Dict[str, Any]:
    ...
    raise RuntimeError(f"DB upload failed after {max_retries} attempt(s): {last_exc}") from last_exc
