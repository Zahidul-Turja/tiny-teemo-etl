...
Responsibilities
----------------
1. Persist ETLJobResult objects (survives server restarts).
2. Publish progress events to a per-job Redis channel so WebSocket
   clients receive live updates without polling.
3. Idempotency helper — store/check a content-addressed job_id so that
   an identical request never spawns a duplicate job.

Key layout
----------
job:<job_id>         →  JSON-serialised ETLJobResult          (string, TTL 24h)
job:idem:<hash>      →  job_id                                 (string, TTL 24h)
channel: etl:<job_id> → pub/sub channel for progress events
"""
...
def save_job(result: ETLJobResult) -> None:
    r = _client()
    r.set(f"job:{result.job_id}", result.model_dump_json(), ex=int(_TTL.total_seconds()))
