"""
ETL API endpoints — upgraded with:
  - Celery background tasks (replaces FastAPI BackgroundTasks)
  - Idempotent job submission (same request → same job_id, no duplicate work)
  - WebSocket for live progress streaming via Redis pub/sub
  - Persistent job store (Redis — survives server restarts)
"""
...
async def run_job(request: ETLJobRequest) -> JSONResponse:
    ...
    if existing_job_id:
        ...
        if cached and cached.message not in ("queued", "running"):
            return JSONResponse(
                status_code=status.HTTP_200_OK if cached.success else status.HTTP_422_UNPROCESSABLE_ENTITY,
...
async def get_job_status(job_id: str) -> JSONResponse:
    ...
    http_status = (
        status.HTTP_202_ACCEPTED if result.message in ("queued", "running") else status.HTTP_200_OK
    )
    return JSONResponse(status_code=http_status, content=result.model_dump())
...
async def get_all_jobs() -> JSONResponse:
    ...
    return JSONResponse(
        status_code=status.HTTP_200_OK,
        content={"success": True, "total": len(jobs), "jobs": [j.model_dump() for j in jobs]},
    )
...
async def get_job_logs(job_id: str) -> JSONResponse:
    ...
    return JSONResponse(
        status_code=status.HTTP_200_OK,
        content={"success": True, "job_id": job_id, "event_count": len(events), "events": events},
    )
...
async def download_invalid_rows(job_id: str) -> FileResponse:
    ...
    return FileResponse(path=path, media_type="text/csv", filename=f"{job_id}_invalid_rows.csv")
