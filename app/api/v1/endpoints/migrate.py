"""
DB-to-DB migration endpoints — powered by Celery + idempotent job store.
Replaces the old BackgroundTasks + in-memory JOB_STORE approach.
"""
import json
import uuid
...
def _new_pending(job_id: str) -> ETLJobResult:
    r = ETLJobResult(job_id=job_id, success=False, message="queued",
...
async def preview_source_schema(source: DatabaseSource) -> JSONResponse:
    try:
        ...
        return JSONResponse(status_code=200, content={
            "success": True, "column_count": len(schema), "columns": schema,
        })
    except Exception as exc:
        raise HTTPException(status_code=400, detail=f"Could not read source schema: {exc}")
...
async def run_migration(request: DBMigrationRequest) -> JSONResponse:
    ...
    if existing:
        ...
        if cached and cached.message not in ("queued", "running"):
            return JSONResponse(status_code=200 if cached.success else 422,
    ...
    if existing:
        ...
        if cached:
            return JSONResponse(status_code=202, content={
                "success": True, "message": "Duplicate — returning existing job.",
                "job_id": existing, "status": cached.message, "idempotent": True,
            })
